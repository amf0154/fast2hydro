"""
Этот модуль реализует маршруты для работы с моделью Хольта-Винтерса в FastAPI, включая обучение модели,
выполнение предсказаний, получение сводной информации и расчет метрик. Основные функции:

1. **train_holtwinter_model** - обучает модель Хольта-Винтерса на предоставленных данных.
2. **predict_holtwinter_model** - выполняет предсказания на основе обученной модели.
3. **get_model_summary** - предоставляет сводную информацию о параметрах и качестве обученной модели.
4. **calculate_metrics_from_json_endpoint** - рассчитывает метрики погрешности между реальными
и прогнозируемыми значениями.

Каждый маршрут обрабатывает соответствующий запрос и возвращает структурированные ответы,
упрощая взаимодействие между сервером и клиентом.
"""
