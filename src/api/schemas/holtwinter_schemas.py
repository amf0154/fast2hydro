"""
Этот модуль содержит схемы запросов и ответов для обучения и предсказания модели Holt-Winters с использованием FastAPI.
Он включает следующие модели:
1. **TrainHoltWintersModelRequest** - описывает входные параметры для обучения модели Holt-Winters.
2. **PredictHoltWintersModelRequest** - описывает входные параметры для выполнения предсказаний на
основе обученной модели.
3. **PredictHoltWintersModelResponse** - определяет формат ответа при выполнении предсказания, включая информацию
об успехе и результаты предсказания.
4. **TrainHoltWintersModelResponse** - описывает ответ при обучении модели, указывая на успех операции и возможное
сообщение об ошибке.
5. **ModelSummary** - содержит сводную информацию о модели после обучения, такие как AIC, BIC, продолжительность
обучения и параметры модели.
6. **MetricsRequest** - описывает входные данные для расчета метрик, включая реальные и прогнозируемые значения.
7. **MetricsResponse** - определяет ответ с рассчитанными метриками, такими как средняя абсолютная ошибка (MAE),
корень из средней квадратичной ошибки (RMSE) и средняя абсолютная процентная ошибка (MAPE).

Модуль упрощает обмен данными между API и клиентом, обеспечивая структурированный формат запросов и ответов для
работы с моделью Holt-Winters.
"""
